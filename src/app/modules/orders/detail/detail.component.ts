...
// import { SetProductsInOrderAction, SetPaymentOrderAction } from '../store/order.action';
...
export class DetailComponent extends Destroyer implements OnInit {
  ...
  private listenProductInOrder(orderId: string) {
    this.products$ = this.store.select(fromOrderSelectors.getOrderInProducts)
      .pipe(this.closeOnDestroy$());
  }

  private listenPayment(): void {
    this.payment$ = this.store.select(fromOrderSelectors.getPaymentOrder)
      .pipe(
        this.closeOnDestroy$()
      );
  }
